...
function validateReservation(req, res, next) {
  ...
  if (
    ...
    next({
      status: 400,
      message: "Need a valid (mobile_number) phone number!",
    });
  } else if (!reservation_date || regExp.test(reservation_date)) {
    next({
      status: 400,
      message: "Reservation date (reservation_date) is missing!",
    });
  } else if (Date.parse(reservation_date) < Date.now()) {
    next({
      status: 400,
      message: "Reservation (reservation_date) needs to be on a future date!",
    });
  } else if (new Date(reservation_date).getDay() + 1 === 2) {
    next({
      status: 400,
      message:
        "We are closed on Tuesdays! Pick a day (reservation_date) when we are open.",
    });
  } else if (!reservation_time || regExp.test(temp_reservation_time)) {
    next({
      status: 400,
      message: "(reservation_time) is missing or not a time!",
    });
  } else if (temp_reservation_time < 1030) {
    next({
      status: 400,
      message:
        "Reservation (reservation_time) cannot be before business hours!",
    });
  } else if (temp_reservation_time > 2130) {
    next({
      status: 400,
      message:
        "Reservation (reservation_time) cannot be less than one hour before business closing!",
    });
...
async function isTimeTaken(req, res, next) {
  ...
  const data = await service.findByDateAndTime(
    reservation_date,
    reservation_time
  );
  if (data.length)
    next({
      status: 400,
      message: "Reservation already scheduled for this time!",
    });
